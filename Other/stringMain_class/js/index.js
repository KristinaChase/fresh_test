...
function camelCaseConverter1(string) {
    ...
    //сначала перевела каждую букву в нижний регистр, потом каждую первую букву в верхний регистр и соеденил в одну строку, получился PascalCase
    let camelCaseStr = PascalCaseStr.split().map(el => el[0].toLowerCase()+el.substring(1)).join('');
    //потом перевела первую букву в нижний.
    return camelCaseStr;
}
...
function spamDetected(string) {
    ...
    //в массиве ищем, есть ли true и возвращаем true, если нет - строку
}
...
function searchInTheTrash(trashString) {
    ...
    //взяла а строки все буквы(латиница и кирилица для след задания) и пробелы, 
    //полученный массив перевела в строку с множеством пробелов, потом обратно в массив почистив от лишних пробело и перевел все в строку с нормальными пробелами
    return normalString;
}
...
function averageWordLength(string) {
    ...
    //полученую строку перевелf в массив для кол-ва слов
    let totalWord = arrayStr.join('');
    //далее массив перевелf в одну строку без пробелов для кол-ва букв
    let averageWord = (totalWord.length/=arrayStr.length);
    //ну и подсчиталf с помощью длин предыдущих значений
    return averageWord;
}
//console.log(averageWordLength(searchInTheTrash(str2))); 
//отправилf дополнительно на чистку от мусора=)
